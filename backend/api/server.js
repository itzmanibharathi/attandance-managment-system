...
// Google automatically gives the service account – NO KEY NEEDED!
admin.initializeApp();
const db = admin.firestore();

// Cloudinary config from Firebase secret (safe)
cloudinary.config({
...
const upload = multer({ dest: "/tmp" }); // Cloud Functions uses /tmp
...
// ── HELPER FUNCTIONS (exactly same as yours) ─────────────────────────────────
...
app.get("/", (req, res) => {
  res.send("Attendance backend running on Google servers – 100% safe!");
});
...
app.post("/students", upload.single("photo"), async (req, res) => {
  try {
    ...
    res.json({ id: ref.id, message: "Student added" });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
// Attendance routes (exactly your code)
...
// Export as Cloud Function (2nd gen – fast & free)
